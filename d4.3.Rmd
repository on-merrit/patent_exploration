...
```{r}
oa_df <- readr::read_csv("data/npl_oa_2008_2020.csv") %>%
...
oa_all <- oa_df %>%
  group_by(is_oa, year) %>%
  summarise(n = n_distinct(family_id)) %>%
  filter(year < 2021)
...
```{r get family id pubilcation year}
bg_patent_oa <- 
  bq_table("api-project-764811344545", "tmp", "patent_oa")
