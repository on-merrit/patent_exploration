...
oa_cat <- readr::read_csv("data/oa_patent_matched_md_cat.csv") %>%
  ...
  mutate(provider_cat = factor(provider_cat, levels=c("Journal only", "Journal & Repository", "Repository only")))
...
ggplot(oa_yrl_cat, aes(x = year, y = n)) +
  ...
  labs(x = "Publication Year", y = "OA NPLs") +
...
oa_shares_by_year %>% 
  select(year, oa_share_npl = oa_share) %>%
  inner_join(upw_oa_by_year) %>%
  select(year, `Unpaywall Total` = prop, `NPL subset` = oa_share_npl) %>%
  pivot_longer(c(`Unpaywall Total`,  `NPL subset`)) %>%
